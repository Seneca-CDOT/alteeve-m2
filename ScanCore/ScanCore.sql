...
-- This stores decisions made by ScanCore.
CREATE TABLE decisions (
	decision_uuid			uuid						primary key,
	decision_host_uuid		uuid						not null,	-- UUID of the machine that made the decision
	decision_order			numeric						not null,	-- A number representing the decisions' order of execution, i.e., 1 is executed first, 2 is executed next, n is executed last
	decision_parameters		text						not null,	-- Stringified perl hash that contains the parameters required for AN:Tools:Action->act()
	modified_date			timestamp with time zone	not null,

	FOREIGN KEY(decision_host_uuid) REFERENCES hosts(host_uuid)
);
ALTER TABLE decisions OWNER TO #!variable!user!#;

CREATE TABLE history.decisions (
	history_id				bigserial,
	decision_uuid			uuid,
	decision_host_uuid		uuid,
	decision_order			numeric,
	decision_parameters		text,
	modified_date			timestamp with time zone
);
ALTER TABLE history.decisions OWNER TO #!variable!user!#;

CREATE FUNCTION history_decisions() RETURNS trigger
AS $$
DECLARE
	history_decisions RECORD;
BEGIN
	SELECT INTO history_decisions * FROM decisions WHERE decision_uuid = new.decision_uuid;
	INSERT INTO history.decisions
		(
			decision_uuid,
			decision_host_uuid,
			decision_order,
	...
	VALUES
		(
			history_decisions.decision_uuid,
			history_decisions.decision_host_uuid,
			history_decisions.decision_order,
			history_decisions.decision_parameters,
			history_decisions.modified_date
		);
	RETURN NULL;
END;
$$
LANGUAGE plpgsql;
ALTER FUNCTION history_decisions() OWNER TO #!variable!user!#;

CREATE TRIGGER trigger_decisions
	AFTER INSERT OR UPDATE ON decisions
	FOR EACH ROW EXECUTE PROCEDURE history_decisions();
